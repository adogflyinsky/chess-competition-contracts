...
contract Prize is IPrize, ERC721Enumerable {
    ...
    struct PrizeConfig {
        ...
        uint256 amount;
        uint8[] ratio; // Purpose is for spending all.

        bool isActive; 
    }

    mapping(uint256 => PrizeConfig) public prizes;

    function mintTo(address to, uint256 taskId, uint256 amount, uint8[] memory ratio) external returns (uint256) {
        ...
        uint8 sum = 0;
        for (uint8 i = 0; i < ratio.length;  i++) {
            sum += ratio[i];
        }
        require(sum == 100, "sum of ratio should be equal 1");
        _tokenIdTracker.increment();    
        uint256 token_id = _tokenIdTracker.current();
        prizes[token_id] = PrizeConfig(msg.sender, taskId, amount, ratio, false);
        _mint(to, token_id);
        return token_id;
    }
